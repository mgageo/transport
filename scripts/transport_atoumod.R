# <!-- coding: utf-8 -->
#
# les réseaux de bus de la région Normandie
#
...
#
# source("geo/scripts/transport.R");atoumod_jour()
atoumod_jour <- function(reseau = "atoumod", force = TRUE) {
  library(tidyverse)
  library(rio)
  library(archive)
  carp()
...
#
# source("geo/scripts/transport.R");atoumod_dl()
atoumod_dl <- function(reseau = "atoumod", force = TRUE) {
  library(tidyverse)
  library(rio)
  library(archive)
  carp()
...
#
# source("geo/scripts/transport.R");atoumod_reseaux()
atoumod_reseaux <- function() {
  library(tidyverse)
  library(rio)
  Tex <<- FALSE
  Wiki <<- FALSE
  df <- atoumod_agency_lire() %>%
    filter(agency_id != "") %>%
...
#    atoumod_gtfs_reseau(Reseau, agency_id, gtfs_dir)
    config_xls(Reseau)
...
#
# source("geo/scripts/transport.R");atoumod_breizhgo()
atoumod_breizhgo <- function() {
  library(tidyverse)
  library(rio)
  Tex <<- FALSE
  Wiki <<- FALSE
...
# source("geo/scripts/transport.R");atoumod_gtfs_reseau_shapes2ogr()
atoumod_gtfs_reseau_shapes2ogr <- function(reseau = "rmat") {
  library(tidyverse)
  library(rio)
...
# source("geo/scripts/transport.R");atoumod_gtfs_reseaux(agency = "TIBUS")
# source("geo/scripts/transport.R");atoumod_gtfs_reseaux(agency = "AXEOBUS")
# source("geo/scripts/transport.R");atoumod_gtfs_reseaux(agency = "QUB")
# source("geo/scripts/transport.R");atoumod_gtfs_reseaux(reseau = "Kiceo")
# setwd("d:/web");source("geo/scripts/transport.R");atoumod_gtfs_reseaux(reseau = "breizhgo56")
atoumod_gtfs_reseaux <- function(reseau = "Kiceo") {
  library(tidyverse)
  library(rio)
...
# source("geo/scripts/transport.R");atoumod_gtfs_reseau("auray", "AURAYBUS")
atoumod_gtfs_reseau <- function(reseau, agency_id, gtfs_dir) {
  library(tidyverse)
  library(rio)
  library(stringr)
  library(archive)
...
# source("geo/scripts/transport.R");atoumod_jour()
atoumod_jour_v0 <- function() {
  atoumod_routes()
#  stop('***')
  atoumod_shapes()
  atoumod_star()
}
# source("geo/scripts/transport.R");atoumod_gtfs()
atoumod_gtfs <- function() {
  library(tidyverse)
  library(rio)
  carp()
...
# source("geo/scripts/transport.R");atoumod_gtfs_ctime()
atoumod_gtfs_ctime <- function() {
  library(tidyverse)
  library(rio)
...
# source("geo/scripts/transport.R");atoumod_gtfs_trips()
atoumod_gtfs_trips <- function() {
  library(tidyverse)
  library(rio)

  tt <- gtfs_lire() %>%
    glimpse()
#  atoumod_gtfs_routes(tt)
#  atoumod_gtfs_shapes(tt)
  gtfs_trips(tt)
}
# source("geo/scripts/transport.R");atoumod_agency_lire()
atoumod_agency_lire <- function() {
  library(tidyverse)
  library(rio)
...
# source("geo/scripts/transport.R");atoumod_agency()
atoumod_agency <- function() {
  library(tidyverse)
  library(readxl)
  carp()
  df <- atoumod_agency_lire()
...
#
# la validation des routes
# source("geo/scripts/transport.R");atoumod_routes()
atoumod_routes <- function() {
  library(tidyverse)
  carp()
  df <- gtfs_routes()
  df1 <- df %>%
...
atoumod_shapes <- function() {
  carp()
  df <- gtfs_shapes_verif()
}
#
# la validation des voyages/arrets
# source("geo/scripts/transport.R");atoumod_gtfs_trips_stops()
atoumod_gtfs_trips_stops <- function() {
  carp()
...
# source("geo/scripts/transport.R");atoumod_star_routes()
atoumod_star_routes <- function() {
  library(tidyverse)
  library(stringr)
  odDir <<- sprintf("%s/web/geo/TRANSPORT/korrigo", Drive)
  atoumod_routes.df <- gtfs_routes()
  odDir <<- sprintf("%s/web/geo/TRANSPORT/STAR/GTFS", Drive)
  star_routes.df <- gtfs_routes()
  df <- atoumod_routes.df %>%
...
#
# la comparaison sur le réseau STAR
atoumod_star_stops <- function() {
  library(tidyverse)
  library(stringr)
  odDir <<- sprintf("%s/web/geo/TRANSPORT/korrigo", Drive)
  atoumod_stops.df <- gtfs_stops()
...
#
# source("geo/scripts/transport.R");atoumod_stops()
atoumod_stops <- function() {
  library(tidyverse)
  library(stringr)
  ...
  nc2 <-  nc %>%
    filter(NOM_REG == "BRETAGNE") %>%
    glimpse()
  dsn <- sprintf("%s/atoumod_stops.Rds", odDir)
  saveRDS(nc2, dsn)
}
#
# lecture du fichier
# source("geo/scripts/transport.R"); df <- atoumod_stops_lire()
atoumod_stops_lire <- function() {
  dsn <- sprintf("%s/atoumod_stops.Rds", odDir)
  carp("dsn: %s", dsn)
  df <- readRDS(dsn) %>%
    st_drop_geometry() %>%
...
#
# les routes osm
# source("geo/scripts/transport.R"); df <- atoumod_osm_routes_lire(force_osm = TRUE) %>% glimpse()
atoumod_osm_routes_lire <- function(force_osm = TRUE) {
...
# source("geo/scripts/transport.R");atoumod_stops_valid()
atoumod_stops_valid <- function() {
  nc <- atoumod_stops_lire()
# on enlève la SNCF
...
# source("geo/scripts/transport.R");atoumod_lignes()
atoumod_lignes <- function() {
  library(sf)
  nc <- atoumod_lignes_lire() %>%
...
# source("geo/scripts/transport.R");atoumod_multi_lire()
atoumod_multi_lire <- function() {
  library(rio)
...
atoumod_gtfs_stops_lire <- function() {
  library(rio)
  fic <- "stops"
  dsn <- sprintf("%s/%s.txt", odDir, fic)
  carp("dsn: %s", dsn)
...
# source("geo/scripts/transport.R");atoumod_stops_diff()
atoumod_stops_diff <- function() {
  library(rio)
  gtfs.df <- atoumod_gtfs_stops_lire()
  multi.df <- atoumod_multi_lire()
  df <- gtfs.df %>%
