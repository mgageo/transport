...
#  stop("*****")
  if (nrow(df2) > 0) {
    level0 <- ""
    for (i2 in 1:nrow(df2)) {
      dsn <- sprintf("%s/gtfs2osm_relations_routemaster_level0_%s.txt", osmDir, df2[[i2, "ref"]])
      carp("dsn: %s", dsn)
      l <- readLines(dsn)
      level0 <- c(level0, l)
    }
...
#    mutate(`ref:network.gtfs` = `ref:network.osm`) %>%
  ...
  carp("osm et gtfs: les communs nrow: %s", nrow(df1))
...
#    confess("osm et gtfs: ref_network nb: %s", nrow(df12))
    df31 <- df12 %>%
      dplyr::select(ends_with(".osm")) %>%
      rename_with(~str_remove(., ".osm")) %>%
      clean_names()
...
diff_osm_routes_tag_shape <- function(force = TRUE, OsmChange = FALSE) {
  library(tidyverse)
  library(data.table)
  library(sf)
  library(janitor)
...
diff_relations_route_tag_shape_potentiel <- function(df31, force = TRUE, OsmChange = FALSE) {
  library(tidyverse)
  library(data.table)
  library(sf)
  library(janitor)
...
diff_relations_route_tags <- function(force = TRUE, OsmChange = FALSE, Debug = FALSE) {
  library(tidyverse)
  library(data.table)
  library(sf)
  library(janitor)
...
#       "network:wikidata",
  ...
  for (i1 in 1:nrow(df1)) {
    id <- df1[[i1, "@id.osm"]]
    carp("diff_objects_tags() id: %s", id)
# pour l'encodage <>
    if (id != "12") {
#      next
    }
# IZILO
    if (id %in% c("12305674", "9211303") ) {
      glimpse(df1[i1, ]); stop("*****")
    }
# Coralie
    if (id %in% c("14241185", "14240639") ) {
#      glimpse(df1[i1, ]); stop("*****")
    }
